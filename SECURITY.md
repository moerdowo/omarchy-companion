...
include the Grok Chief and Omarchy versions, the smallest reproduction you can
...
plugin/config migration, unintended writes outside Grok Chief's documented
...
process. Grok Chief does not make that boundary narrower.

An explicit bubble submission or `omarchy-shell grokchief order …` call runs
...
terminal. Apart from the agent it launches, Grok Chief's own runtime makes no
