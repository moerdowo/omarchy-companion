...
include the Omarchief and Omarchy versions, the smallest reproduction you can
...
plugin/config migration, unintended writes outside Omarchief's documented
...
process. Omarchief does not make that boundary narrower.

An explicit bubble submission or `omarchy-shell omarchief order …` call runs
...
terminal. Apart from the agent it launches, Omarchief's own runtime makes no
