...
the shared scale is chosen as the largest one at which the tallest frame
anywhere still fits its cell. Each frame is then seated on its own lowest
visible pixel, so feet land on the ground and only what should rise above
the creature does.
...
def strip_edge_bleed(path, share=0.08):
    ...
    for line in out.splitlines():
        if "gray(255)" not in line:
            continue
        cols = line.split()
        m = re.match(r"(\d+)x(\d+)\+(\d+)\+(\d+)", cols[1])
        w, h, x, y = (int(v) for v in m.groups())
        parts.append({"area": int(cols[3]), "top": y, "bottom": y + h})
    if len(parts) < 2:
        return
...
class Source:
    ...
    numbers are all it takes to place every source at one shared scale.
    """
    ...
    def __init__(self, name, frames):
        ...
        self.reference = self.frames[0][1][0]
        self.span = max(b[3] + b[1] for _, b in self.frames) - self.top

    def cube_limit(self, room):
        """The widest this cube may be drawn before its tallest frame
        stops fitting — expressed in cell pixels."""
        return room * self.reference / max(1, self.span)
...
def cut_grid(sheet, tmp, cols, rows, tag):
    ...
    sw, sh = (int(v) for v in run(["magick", "identify", "-format", "%w %h", str(sheet)]).split())
    gw, gh = sw // cols, sh // rows
    out = []
    for r in range(rows):
        band = []
        for c in range(cols):
...
def place(src, dst, box, top, scale, mirror=False):
    ...
    run(["magick", str(src), "-crop", f"{img_w}x{span}+0+{top}", "+repage"] +
        (["-flop"] if mirror else []) +
        ["-resize", f"{round(img_w * scale)}x{round(span * scale)}!",
...
def main():
    ...
    walk_args = opt("--walk", 2)
    idle_args = opt("--idle", 1)
    act_args = opt("--activities", 4)
    pose_args = opt("--poses", 1)
    if not walk_args or not idle_args:
        sys.exit(__doc__)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        walk_sheet, walk_n = Path(walk_args[0]), int(walk_args[1])
        prepared = tmp / "walk.png"
        prepare(walk_sheet, prepared)
        sw = int(run(["magick", "identify", "-format", "%w", str(walk_sheet)]))
        grid = sw // walk_n
        walk_frames = []
        ...
        prepare(idle_args[0], idle_prepared)
        ...
        act_rows, act_names = [], []
        if act_args:
            sheet, cols, rows, names = Path(act_args[0]), int(act_args[1]), int(act_args[2]), act_args[3].split(",")
        ...
        limits = {k: s.cube_limit(room) for k, s in sources.items()}
        ...
        poses = {}
        if pose_args:
            for entry in pose_args[0].split(","):
                if "=" not in entry:
                    continue
                name, where = entry.split("=", 1)
                r, c = (int(v) for v in where.split(":"))
                if r < len(act_cells) and c < len(act_cells[r]):
                    poses[name.strip()] = act_cells[r][c]
                else:
                    sys.exit(f"pose {name} points outside the activity sheet")
